...
describe('Reader', () => {
  ...
  describe('reading position bookmark', () => {
    it('writes the current paragraph to localStorage when it becomes 50%+ visible', async () => {
      ...
      fireIntersection([{ target: p1, isIntersecting: true, intersectionRatio: 0.6 }])
    ...
    it('does not update the bookmark for sub-50% visibility', async () => {
      ...
      fireIntersection([{ target: p1, isIntersecting: true, intersectionRatio: 0.2 }])
    ...
    it('scrolls to the saved paragraph on mount when a bookmark exists', async () => {
      setBookmark('tsundoku-test', { chapterId: '00-test-chapter-1', paragraphId: 'p1' })
      gotoReader('/reader/tsundoku-test?chapter=00-test-chapter-1&paragraph=p0')
      render(<App />)
      const p1 = await findParagraphByText('猫が好きです。')
    ...
    it("does not overwrite another book's bookmark", async () => {
      ...
      render(<App />)
      const p1 = await findParagraphByText('猫が好きです。')
      fireIntersection([{ target: p1, isIntersecting: true, intersectionRatio: 0.9 }])
